...
public class ARWallPresenter : MonoBehaviour
{
    ...
    private void FitToScreen()
    {
        ...
        if (isRealDevice)
        {
            // ИСПРАВЛЕНИЕ: Уменьшаем масштабирование для точного покрытия
            width *= 1.10f;  // Уменьшили с 1.50f до 1.10f для устранения растягивания
            height *= 1.10f; // Уменьшили с 1.50f до 1.10f для устранения растягивания

            // На реальных устройствах размещаем плоскость близко для максимального покрытия
            distance = arCamera.nearClipPlane + 0.01f;

            Debug.Log($"📱 Реальное устройство: точное покрытие экрана - screenAspect={screenAspect:F3}");
        }
        else
        {
