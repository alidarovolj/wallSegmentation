    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 correctAspectUV(float2 uv)
            {
                ...
                {
                    // ОБЛЕГЧЕННАЯ коррекция для портретного экрана
                    // Уменьшаем степень коррекции чтобы маска покрывала больше экрана
                    float aspectRatio = lerp(1.0, screenAspect, 0.3); // Смягчили с 0.462 до ~0.8
                    correctedUV.y = (correctedUV.y - 0.5) * aspectRatio + 0.5;
                    
                    // Убираем дополнительный сдвиг
                    // correctedUV.y += 0.05; // УБРАЛИ сдвиг
                }
